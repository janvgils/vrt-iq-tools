...
int main(int argc, char* argv[])
{
    ...
    // HydraSDR
    struct hydrasdr_device* device = 0;
    char hydrasdr_version[255 + 1];
    ...
    if (vm.count("help")) {
        std::cout << boost::format("HydraSDR RX samples to VRT. %s") % desc << std::endl;
        std::cout << std::endl
                  << "This application streams data from a single channel of an HydraSDR "
                     "device to VRT.\n"
                  << std::endl;
        return ~0;
    }
    ...
    enum hydrasdr_sample_type sample_type_val = HYDRASDR_SAMPLE_INT16_IQ;
    ...
    // check sample rates
    hydrasdr_get_samplerates(device, &nsrates, 0);
    supported_samplerates = (uint32_t *) malloc(nsrates * sizeof(uint32_t));
    hydrasdr_get_samplerates(device, supported_samplerates, nsrates);
    ...
    // set sample rate
    if (hydrasdr_set_samplerate(device, sample_rate_u32) != HYDRASDR_SUCCESS) {
        fprintf(stderr, "hydrasdr_set_samplerate() failed: %d\n", sample_rate_u32);
        exit(1);
    }
    ...
    if (hydrasdr_board_partid_serialno_read(device, &read_partid_serialno) != HYDRASDR_SUCCESS) {
        fprintf(stderr, "hydrasdr_board_partid_serialno_read() failed\n");
        exit(1);
    } else
    ...
    result = hydrasdr_version_string_read(device, &hydrasdr_version[0], 255);
    if (result != HYDRASDR_SUCCESS) {
        fprintf(stderr, "hydrasdr_version_string_read() failed: %s (%d)\n",
            hydrasdr_error_name((hydrasdr_error)result), result);
    }

    if (strcmp(hydrasdr_version,"HydraSDR RFOne v1.0.2 2025-07-18") == 0)
        fw_version = 1;
    else if (strcmp(hydrasdr_version,"HydraSDR RFOne v1.0.2 2025-07-18") == 0)
        fw_version = 0;

    fprintf(stderr, "Firmware Version: %s (hydrasdr_to_vrt: %i)\n", hydrasdr_version, fw_version);

    result = hydrasdr_set_packing(device, (uint8_t)0);
    if (packing) {
        result = hydrasdr_set_packing(device, (uint8_t)packing);
        if( result != HYDRASDR_SUCCESS ) {
    ...
    result = hydrasdr_set_rf_bias(device, (int)bias_tee);
    if( result != HYDRASDR_SUCCESS ) {
    ...
    result = hydrasdr_si5351c_read(device, 0, &register_value); 
    if (result == HYDRASDR_SUCCESS)
    ...
    result = hydrasdr_si5351c_read(device, 15, &register_value); 
    if (result == HYDRASDR_SUCCESS)
    ...
    if( hydrasdr_start_rx(device, rx_callback, NULL) != HYDRASDR_SUCCESS ) {
        fprintf(stderr, "hydrasdr_start() failed.\n");
        exit(1);
    }

    if( hydrasdr_set_freq(device, (uint32_t)freq) != HYDRASDR_SUCCESS ) {
        fprintf(stderr, "hydrasdr_set_freq() failed.\n");
        exit(1);
    }
    ...
    /* switch bias-tee off */
    hydrasdr_set_rf_bias(device, 0);

    /* clean up */
    if( hydrasdr_stop_rx(device) != HYDRASDR_SUCCESS ) {
        fprintf(stderr, "hydrasdr_stop() failed\n");
    }

    if( hydrasdr_close(device) != HYDRASDR_SUCCESS ) {
        fprintf(stderr,"hydrasdr_close() failed\n");
    }
